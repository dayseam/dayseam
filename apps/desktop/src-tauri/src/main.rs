...
fn main() {
    ...
    // **MAS-3a.** App Store SKU must not register the in-app updater or
    // process-relaunch plugins; updates ship via Apple only.
    let builder = {
        #[cfg(not(feature = "mas"))]
        {
            builder
    ...
    let builder = builder.setup(|app| {
        ...
        let about_metadata = AboutMetadataBuilder::new()
            ...
            .build();
        let app_submenu_builder = SubmenuBuilder::new(app, "Dayseam").about(Some(about_metadata));
        #[cfg(not(feature = "mas"))]
        let app_submenu_builder = app_submenu_builder.item(&check_updates).separator();
        ...
        let theme_system = MenuItemBuilder::with_id("view_theme_system", "System").build(app)?;
