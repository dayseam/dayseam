...
| **v0.3. Report polish + deferred-findings hardening** | [`2026-04-20-v0.3-report-polish-hardening.md`](./2026-04-20-v0.3-report-polish-hardening.md) | Not a feature phase: no new sources, no new IPC surface. Consumes the 22 Mediums the v0.2 capstone review deferred to umbrella issues [#83](https://github.com/vedanthvdev/dayseam/issues/83)–[#87](https://github.com/vedanthvdev/dayseam/issues/87). Headline deliverables: **per-kind report sections** (events grouped under `Commits` / `Merge requests` / `Jira issues` / `Confluence pages` instead of a single `COMMITS` catchall), **Atlassian Reconnect dialog parity** (dead chip wired through to a pre-filled `AddAtlassianSourceDialog`), a silent-failure sweep across the Confluence normaliser + Jira walker + orchestrator temporal ordering, cross-source consistency work (`EntityRef.kind` discriminant, `workspace` entity, 5xx classification symmetry), and a test-quality floor raise including a proc-macro-based `#[serde(default)]` class detector (the lesson from DOG-v0.2-04). Seven PRs (DAY-85..DAY-91); intermediate PRs carry `semver:none` so only the capstone auto-releases as `v0.3.0`. |
