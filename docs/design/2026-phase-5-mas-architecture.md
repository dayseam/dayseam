...
**MAS-8d** (automated upload) lives in [`mas-connect-upload.yml`](../../.github/workflows/mas-connect-upload.yml) — **separate** from direct [`release.yml`](../../.github/workflows/release.yml), with job-level **`continue-on-error: true`** unless the team explicitly couples them (plan).
...
- [x] **Local MAS build helper (MAS-8b):** [`scripts/release/mas/README.md`](../../scripts/release/mas/README.md) + [`build-mas-app.sh`](../../scripts/release/mas/build-mas-app.sh) — **replace or delete** when the **MAS-8d** pipeline **subsumes** it (see **`MAS-8d-followup`** for store **`.pkg`** in CI — disposable scaffold, not a second release pipeline).
- [x] **DAY-195 changelog preflight on MAS packaging CI (MAS-8c):** [`mas-package-verify.yml`](../../.github/workflows/mas-package-verify.yml) **`bash -n`**'s and runs [`check-unreleased-for-semver-pr.sh`](../../scripts/release/check-unreleased-for-semver-pr.sh) with mock **`semver:patch`** after a full-history checkout — same **`extract-release-notes.sh`** gate as PR **`changelog-semver-pr`**, so **`CHANGELOG.md`** regressions fail on tag / schedule / **`workflow_dispatch`** before **`tauri build`**. [`ci.yml`](../../.github/workflows/ci.yml) **`shell-scripts`** **`bash -n`**'s the script on PRs; **`minor` / `major`** slices stay PR-gated.
- [x] **App Store Connect upload (MAS-8d):** [`mas-connect-upload.yml`](../../.github/workflows/mas-connect-upload.yml) + [`MAS-CONNECT-UPLOAD.md`](../../docs/release/MAS-CONNECT-UPLOAD.md) — **`workflow_dispatch`**, **`DAYSEAM_ASC_*`** secrets, **TestFlight** via **Transporter** + **`upload-testflight-build`**, **`continue-on-error`**, default **`dry_run: true`** preflight [`mas-connect-upload-preflight.sh`](../../scripts/release/mas/mas-connect-upload-preflight.sh). **`.pkg` CI** → **`MAS-8d-followup`**.
...
**MAS-8c:** [`mas-package-verify.yml`](../../.github/workflows/mas-package-verify.yml) runs the **DAY-195** [`check-unreleased-for-semver-pr.sh`](../../scripts/release/check-unreleased-for-semver-pr.sh) preflight (mock **`PR_LABELS_JSON`** with **`semver:patch`**, the usual MAS-line cadence) on every **`v*`** tag, weekly **`schedule`**, and **`workflow_dispatch`**, mirroring [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) **`changelog-semver-pr`** so **`CHANGELOG.md`** stays releasable when packaging proof runs off the PR path. **`shell-scripts`** **`bash -n`**'s the same script on PRs; **`minor` / `major`** release-note slices remain exercised via labelled PRs only.

**MAS-8d:** [`mas-connect-upload.yml`](../../.github/workflows/mas-connect-upload.yml) is the **TestFlight-first** App Store Connect upload path for **macOS `.pkg`** (operator-triggered **`workflow_dispatch`**, Apple **Transporter** install on the runner, [`apple-actions/upload-testflight-build`](https://github.com/apple-actions/upload-testflight-build) **v4.1.0** pinned to **`994cd4f`** with **`backend: transporter`**). It is **not** part of [`release.yml`](../../.github/workflows/release.yml); the job uses **`continue-on-error: true`** so Connect failures do not block the direct channel. Runbook: [`MAS-CONNECT-UPLOAD.md`](../../docs/release/MAS-CONNECT-UPLOAD.md). **Store-signed `.pkg` production in CI** remains **`MAS-8d-followup`**.
...
| 2026-05-02 | **MAS-8c:** [`mas-package-verify.yml`](../../.github/workflows/mas-package-verify.yml) DAY-195 mock **`check-unreleased-for-semver-pr.sh`** + full-history checkout; §21 + §20 checklist. MR review: **`bash -n`** in workflow + **`ci.yml`** **`shell-scripts`**, doc wording (**CHANGELOG** / mock **patch** vs PR **minor**/**major**). |
| 2026-05-02 | **MAS-8d:** [`mas-connect-upload.yml`](../../.github/workflows/mas-connect-upload.yml) + [`MAS-CONNECT-UPLOAD.md`](../../docs/release/MAS-CONNECT-UPLOAD.md) + preflight script; §17 / §20 / §21; **MAS-EXPORT** checklist; **ARCHITECTURE** workflows tree; plan risk register (**MAS-8b** scaffold). MR review: **`mas-package-verify.yml`** link in runbook; workspace-canonical **`.pkg`** + ASC secret gate; **`upload-testflight-build`** pin **994cd4f**; preflight exit-code comment; Transporter installer cleanup. |
