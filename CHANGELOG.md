...
- **DAY-268:** Source chips no longer show an inline **✕**; **Rescan** and **Edit** hovers use a stronger neutral highlight. **Delete** lives in every connector **edit** dialog as a full-width red **Delete source…** control that opens the same confirmation as before. On success the chip plays a **collapse-to-centre** exit animation (respecting `prefers-reduced-motion`), a short **Web Audio** “pop” when motion is allowed, and a **success toast** so removal is obvious without sound. If `sources_delete` fails after the animation, the edit dialog and confirmation reopen with the same inline error behaviour as today. ([#268](https://github.com/dayseam/dayseam/issues/268))

- **DAY-266:** Bump **`typescript-eslint`** to **8.59.2** (desktop + e2e). Split **`connectorAccent`** and connector SVG path data into **`connectorAccent.ts`** / **`connectorMarks.ts`**, and moved **`aggregateByKind`** into **`daySummaryChartAggregate.ts`**, so `ConnectorLogo.tsx` and `DaySummaryChart.tsx` only export **components** (plus type-only prop interfaces) — clearing **`react-refresh/only-export-components`** ESLint warnings without changing runtime behaviour.
...
- **DAY-210 / MAS-9c (dogfood evidence scaffold):** [`docs/review/phase-5-mas-review.md`](docs/review/phase-5-mas-review.md) §2 advances head to **`3573d1c`** ([#253](https://github.com/dayseam/dayseam/pull/253)), GitHub compare **`8aaab40...3573d1c`**, PR table row **#253**, and refreshed `git diff --shortstat`; **§5** adds **MAS-9c** operator checklist table (placeholders), **§1** pointer, and **CI** [`mas-package-verify.yml`](.github/workflows/mas-package-verify.yml) / [`mas-sandbox-launch-smoke.sh`](scripts/ci/mas-sandbox-launch-smoke.sh) supplement note. [`docs/plan/2026-phase-5-mas-app-store.md`](docs/plan/2026-phase-5-mas-app-store.md) document history. ([#254](https://github.com/dayseam/dayseam/pull/254)) MR review: **Remaining** line no longer says **MAS-9b** is “above” the header; **§0** bar **A** names **MAS-9c** **§5** table **Pending**; checklist table rows match **§1** prose; **Status** “dogfood pass” wording.
...
- **DAY-210 / MAS-9a (lenses — closeout scaffold):** [`docs/review/phase-5-mas-review.md`](docs/review/phase-5-mas-review.md) §2 advances head to **`8cb1362`** ([#251](https://github.com/dayseam/dayseam/pull/251)), GitHub compare **`8aaab40...8cb1362`**, PR table row **#251**, and refreshed `git diff --shortstat`; **§0** exit bars **A** / **B** set to **Partial** with **MAS-9b** / **MAS-9c** / upload-evidence deferrals; **§3.9** gains **Status** / **Evidence** / **Gap** paragraphs + **`ipc/commands`** **§3.8** cross-ref; **§5** lens-coverage note. ([#252](https://github.com/dayseam/dayseam/pull/252)) MR review: **§3.9** **Status** links **MAS-0b** §3 taxonomy (not vague "§3 columns"); **Evidence** adds **§3.2**; bar **B** defers upload-path evidence to **Appendix A** / **§5** / **MAS-9c**; **ASCII** apostrophe in **plan's**.
...
- **DAY-210 / MAS-9a (lenses — oauth):** [`docs/review/phase-5-mas-review.md`](docs/review/phase-5-mas-review.md) §2 advances head to **`c68282b`** ([#247](https://github.com/dayseam/dayseam/pull/247)), GitHub compare **`8aaab40...c68282b`**, PR table row **#247**, and refreshed `git diff --shortstat`; **§3.5 OAuth** (**Partial**) ties [`ipc/oauth.rs`](apps/desktop/src-tauri/src/ipc/oauth.rs), [`oauth_config.rs`](apps/desktop/src-tauri/src/oauth_config.rs) / [`oauth_session.rs`](apps/desktop/src-tauri/src/oauth_session.rs), **MAS-6b** [`entitlements.mas.plist`](apps/desktop/src-tauri/entitlements.mas.plist) + [`entitlements.mas.md`](apps/desktop/src-tauri/entitlements.mas.md), **MAS-0b** §13–§14, and `OAUTH_LOGIN_*` codes in [`error_codes.rs`](crates/dayseam-core/src/error_codes.rs) with **§3.9** `ipc/oauth` inventory row + **MAS-9c** Canonical smoke item **6**. ([#248](https://github.com/dayseam/dayseam/pull/248)) MR review: correct **`oauth.login.*`** vs **`DayseamError`** mapping (**`InvalidConfig`** vs **`Internal`** vs session **`Failed`** + **`exchange_code`** codes); **§3.2** evidence + §3.7 gap mention **`ipc/oauth`**; **§2.3** rough centres include OAuth loopback.
...
- **DAY-210 / MAS-9a (lenses — errors):** [`docs/review/phase-5-mas-review.md`](docs/review/phase-5-mas-review.md) §2 advances head to **`fe1d079`** ([#245](https://github.com/dayseam/dayseam/pull/245)), GitHub compare **`8aaab40...fe1d079`**, PR table row **#245**, and refreshed `git diff --shortstat`; **§3.2 Errors** (**Partial**) ties [`error.rs`](crates/dayseam-core/src/error.rs), [`error_codes.rs`](crates/dayseam-core/src/error_codes.rs), and bookmark error mapping in [`ipc/commands.rs`](apps/desktop/src-tauri/src/ipc/commands.rs) (`IPC_SECURITY_SCOPED_BOOKMARK_*`, `map_bookmark_materialize_db_error`, sync `internal(...)` paths) with hooks for **§3.3** / **§3.5** / **§3.7**. ([#246](https://github.com/dayseam/dayseam/pull/246))
...
- **DAY-210 / MAS-9a (scaffold):** [`docs/review/phase-5-mas-review.md`](docs/review/phase-5-mas-review.md) is the **MAS-9a** capstone review shell — exit bars **A** / **B**, Canonical MAS smoke pointer, per-plan review lenses (IPC, errors, Keychain, FS, OAuth, subprocesses, capabilities, CSP, **`cfg` / `mas` feature inventory**), findings table, and **MAS-9b** / **MAS-9c** handoff. Fill evidence as the review progresses. [`2026-phase-5-mas-app-store.md`](docs/plan/2026-phase-5-mas-app-store.md) adds stable **`#mas-block-9-capstone`** and **`#canonical-mas-smoke-checklist`** anchors so links from the review doc resolve (MR review).
...
- **DAY-210 / MAS-4b:** Desktop crate [`security_scoped`](apps/desktop/src-tauri/src/security_scoped/mod.rs) wraps Foundation **`NSURL`** APIs for **directory** security-scoped bookmark **create** / **resolve** (incl. stale flag; paths via `fileSystemRepresentation`) and an RAII **`SecurityScopedGuard`** (`startAccessing…` / `stopAccessing…` on drop). **`SecurityScopedGuard::from_bookmark`** resolves persisted bytes then starts access on that URL (sandbox-correct); **`::new(path)`** remains for direct / test builds. Empty bookmark bytes map to **`EmptyBookmarkData`**. **macOS** unit tests cover round-trip, guard write, and validation; **non-macOS** stubs return [`SecurityScopedError::UnsupportedPlatform`](apps/desktop/src-tauri/src/security_scoped/mod.rs). See [`docs/design/2026-phase-5-mas-architecture.md`](docs/design/2026-phase-5-mas-architecture.md) §9.5.

- **DAY-210 / MAS-4a:** SQLite migration [`0007_security_scoped_bookmarks.sql`](crates/dayseam-db/migrations/0007_security_scoped_bookmarks.sql) adds **`security_scoped_bookmarks`** — persistent store for macOS security-scoped directory bookmarks keyed to Local Git scan roots and Markdown sink destinations ([`docs/design/2026-phase-5-mas-architecture.md`](docs/design/2026-phase-5-mas-architecture.md) §9.6). **`bookmark_blob`** is populated when the dialog flow persists grants (**MAS-4e**); rows cascade-delete with their source or sink. **`dayseam-db`** now ships [`build.rs`](crates/dayseam-db/build.rs) that emits `cargo:rerun-if-changed` on the whole **`migrations/`** directory so **new** `.sql` files (not only edits to existing ones) rebuild the crate and refresh the `sqlx::migrate!` embed.
...
- **DAY-210 / MAS-1b:** `desktop-bundle` CI merges **`bundle.createUpdaterArtifacts: false`** into both Tauri builds so the bundler does not require **`TAURI_SIGNING_PRIVATE_KEY`** (release-only) while [`tauri.conf.json`](apps/desktop/src-tauri/tauri.conf.json) keeps **`plugins.updater.pubkey`** for production updater trust.
...
- **DAY-214 (saved report):** inline summary-chart legend and centre
  ...
  inherit`** on the SVG still resolved **`currentColor` to black**.
  Bind vault text with **`style="color: var(--text-normal,
  light-dark(#1f2937, #e5e7eb));"`** on the `<svg>` root (vars resolve
  there reliably, unlike the SVG-internal stylesheet alone), keep
  **`fill: currentColor`**, and set **`fill="currentColor"`** on `<text>`
  when `<style>` is stripped. Re-save a daily note to refresh embedded charts.
